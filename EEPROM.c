...
unsigned char ext_eeprom_24C02_read(unsigned char addr)
{
    ...
    i2c_rep_start();
    //Sending the slave address byte with read but LSB
    i2c_write(SLAVE_READ_EEPROM);
    //Reading the data from SSPBUFF
    data = i2c_read(0);
    //Sending stop bit to make the I2C bus ready for further transmissions
    i2c_stop();
    return data;
}
...
void ext_eeprom_24C02_byte_write(unsigned char addr,char data)
{
    ...
    i2c_write(data);
    //Sending stop bit to make ready the 12C bus for further transmissions
    i2c_stop();
}
